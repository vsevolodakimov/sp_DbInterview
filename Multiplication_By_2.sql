CREATE PROCEDURE [dbo].[Multiplication_By_2]
(
...
BEGIN
	...
	BEGIN
	   ...
	   SELECT @Char = SUBSTRING(@Dividend, @CountAll, 1), -- Вытаскиваем цифру из строки @Dividend  с позиции @CountAll и помещаем её в переменную @Char
              @ResultMult = CAST(@Char AS INT) * 2     -- цифру в переменной @Char умножаем на 2, и результат умножения заноситм в переменную @ResultMult
	...
	-- Последний символ
	SELECT @countElements + 1,
	       CAST(T4.last_figure AS CHAR)
	FROM #Multiplication AS T4 
	WHERE T4.id = @countElements


	/************************************************************************************/

	-- Проверка на первую цифру из числа
	IF @firstSimbol IN ('1', '2', '3', '4')	    
	...
	TRUNCATE TABLE #Multiplication 
	TRUNCATE TABLE #ResultMerger 
...
EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'ХП Умножает на 2 число "@Dividend" записанное в виде строки.' , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'PROCEDURE',@level1name=N'Multiplication_By_2'
GO
