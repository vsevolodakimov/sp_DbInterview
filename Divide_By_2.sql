CREATE PROCEDURE [dbo].[Divide_By_2]
(
...
SET NOCOUNT ON
	...
	DECLARE @Result VARCHAR(MAX), -- Переменная для возвращаеиого значения (Результат деления)
	        ...
	        @Char VARCHAR(1),        -- для помещения сюда цифры по одной
			@ResultMult INT,       -- Результат умножения цифры на 5
			@firstSimbol VARCHAR(1), -- Первый символ в числе @Dividend
			@lastSimbol VARCHAR(1)  -- Последний символ в числе @Dividend
	...
	SET @Result = REPLACE(@Result, ']', '')
     ...
     -- Проверка четное или нечетное число (Если число нечётное то прибавляем в конец символы ",5")
...
EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'ХП делит на 2 любое целое число больше 1, записанное в виде строки' , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'PROCEDURE',@level1name=N'Divide_By_2'
GO
